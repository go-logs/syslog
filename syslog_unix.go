...
func (n *localConn) writeString(framer Framer, formatter Formatter, p Priority, hostname, appName, tag, msg string) error {
	if framer == nil {
	...
	_, err := n.conn.Write([]byte(framer(formatter(p, hostname, appName, tag, msg))))
	return err
}
